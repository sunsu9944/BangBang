...
class MainViewController: UIViewController {
    ...
    func addremmendViewsaddview() { // 페이지마다 뷰와 버튼 생성
        for _ in 0..<3 {
            ...
            itemview.append(UIImageView())
        }
        
        for i in 0..<3 { // 구조 설정 및 추가
            //let xPos = self.view.frame.width * CGFloat(i)
        
            recommendview.addSubview(contents[i])
            recommendview.contentSize.width = view.frame.width * 3
            ...
            itemview[i].layer.masksToBounds = true
    ...
    let picnicview :UIButton = {
        let bt = UIButton()
        bt.layer.cornerRadius = 15
        bt.backgroundColor = UIColor.init(red: 186/255, green: 195/255, blue: 232/255, alpha: 1)
        return bt
    }()
    ...
    let GPSview :UIButton = {
        ...
        bt.layer.cornerRadius = 15
        bt.backgroundColor = UIColor.init(red: 207/255, green: 204/255, blue: 255/255, alpha: 1)
        return bt
    }()
    ...
    let GPSimg: UIImageView = {
        ...
        let img  = UIImage(named: "smartphone.png")
        imgv.image = img
    ...
    func addMainButtons() {
        for i in 0..<3 {
            ...
            case 0:
                ...
                mainbutton[i].backgroundColor = UIColor.init(red: 126/255, green: 224/255, blue: 235/255, alpha: 1)
            case 1:
    ...
    func autolayout(){
        ...
        GPSimg.snp.makeConstraints { make in
            make.leading.equalTo(picnicimg).offset(10)
            make.trailing.equalTo(GPSview).offset(-10)
            make.top.equalTo(GPSview).offset(10)
    ...
    override func viewDidLoad() {
        ...
        festivalscrollView.contentSize.width = view.frame.width * 3
        ...
        LoadFestivalData.shared.fetch {
            ...
            self.festivalCount = LoadFestivalData.shared.festivaltour.count
            self.addfestivalviews()
            
            
            for i in 0..<self.festivalCount{
                
                if let imagepath = LoadFestivalData.shared.festivaltour[i].imagepath{
                    
                    AF.request(imagepath, method: .get).response{ response in
                        
                        switch response.result {
                        case.success(let responseData):
                            self.festivalbutton[i].img.image = UIImage(data: responseData!, scale: 1)
                ...
                guard let start = LoadFestivalData.shared.festivaltour[i].start else { continue }
                guard let end = LoadFestivalData.shared.festivaltour[i].end else {continue}
                self.festivalbutton[i].festivalname.text = LoadFestivalData.shared.festivaltour[i].fname
                self.festivalbutton[i].festivaltime.text = "\(start) ~ \(end)"
            }
        ...
        itemview[0].image = UIImage(named:"경주 투어")
        itemview[1].image = UIImage(named:"어린왕자 투어")
        itemview[2].image = UIImage(named:"이순신 투어")
        ...
        mainbutton[0].addTarget(self, action: #selector(searchDogtravelaction(_:)), for: .touchUpInside)
        mainbutton[1].addTarget(self, action: #selector(searchOldtravelaction(_:)), for: .touchUpInside)
        mainbutton[2].addTarget(self, action: #selector(searchBabytravelaction(_:)), for: .touchUpInside)
        picnicview.addTarget(self, action: #selector(searchpicnictravelaction(_:)), for: .touchUpInside)
        
        
        mainbutton[2].btimg.snp.remakeConstraints { make in
            make.leading.equalTo(mainbutton[2].snp.centerX).offset(90)
            make.trailing.equalTo(mainbutton[2]).offset(-40)
            make.top.equalTo(mainbutton[2]).offset(10)
            make.bottom.equalTo(mainbutton[2]).offset(-10)
        }
        mainbutton[1].btimg.snp.remakeConstraints { make in
            make.leading.equalTo(mainbutton[1].snp.centerX).offset(80)
            make.trailing.equalTo(mainbutton[1]).offset(-30)
            make.top.equalTo(mainbutton[1]).offset(-5)
            make.bottom.equalTo(mainbutton[1]).offset(5)
        }
...
extension MainViewController: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        print(currentpage + 1)
        currentPageLabel.text = "\(currentpage+1) / 3"
    }
}
...
extension MainViewController {
    ...
    @objc func searchDogtravelaction(_ sender: UIButton) {
        type = .tourlist
        ToursLoadData.shared.Dogfetch {
            let vc = resultViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
    
    @objc func searchOldtravelaction(_ sender: UIButton) {
        type = .tourlist
        ToursLoadData.shared.Oldfetch {
            let vc = resultViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
    
    @objc func searchBabytravelaction(_ sender: UIButton) {
        type = .tourlist
        ToursLoadData.shared.Babyfetch {
            let vc = resultViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
    @objc func searchpicnictravelaction(_ sender: UIButton) {
        type = .tourlist
        ToursLoadData.shared.picnicfetch {
            let vc = resultViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
